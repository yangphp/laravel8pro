...
<div class="layui-form layuimini-form">
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="text" name="title" lay-verify="required" lay-reqtext="幻灯片标题不能为空" placeholder="请输入幻灯片标题" value="{{$info->title}}" class="layui-input">
        </div>
    </div>
    <div class="layui-form-item">
        <label class="layui-form-label required">跳转地址</label>
        <div class="layui-input-block">
            <input type="text" name="jumpurl" lay-verify="required" lay-reqtext="跳转地址不能为空" placeholder="请输入跳转地址" value="{{$info->jumpurl}}" class="layui-input">
        </div>
    </div>
    <div class="layui-form-item">
        <label class="layui-form-label required">排序</label>
        <div class="layui-input-block">
            <input type="text" name="swiper_sort" lay-verify="required" lay-reqtext="排序不能为空" placeholder="请输入1-100" value="{{$info->swiper_sort}}" class="layui-input">
            <tip>排序的值越小越靠前</tip>
        </div>
    </div>
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            ...
            <input type="radio" name="swiper_status" value="1" title="显示"  @if ($info->swiper_status == 1)checked="" @endif>
            <input type="radio" name="swiper_status" value="0" title="隐藏" @if ($info->swiper_status == 0)checked="" @endif>
        </div>
    </div>
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <div id="zyupload" class="zyupload">
                <img class="imgurl" src="{{$info->imgurl}}" height="100"/>
                <input id="imgurl" type="hidden" name = 'imgurl'  value="{{$info->imgurl}}" />
            </div>
        </div>
    </div>
    ...
    layui.use(['form'], function () {
        ...
        form.on('submit(saveBtn)', function (data) {
            ...
            $.ajax({
                type:'PUT',
                data:data.field,
                url: "/admin/swiper/"+data.field.id,
